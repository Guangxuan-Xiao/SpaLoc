...
class Family(object):
    ...
    def get_self(self):
        return np.eye(self._n, dtype=np.int32)

    def get_family_of_three(self):
        # family_of_three[x, y, z] iff father[x, y] and mother[x, z].
        father_exp = np.expand_dims(self.father, axis=2)
        mother_exp = np.expand_dims(self.mother, axis=1)
        family_of_three = father_exp * mother_exp
        return family_of_three

    def get_three_generations(self):
        # three_generations[x, y, z] iff parent[x, y] and parent[y, z].
        parent = self.get_parents()
        parent1 = np.expand_dims(parent, axis=2)
        parent2 = np.expand_dims(parent, axis=0)
        three_generations = parent1 * parent2
        return three_generations
