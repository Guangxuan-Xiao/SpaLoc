...
__all__ = [
    ...
    'FamilyTreeDataset', 'GraphTransitivityDataset', 'GraphSelfDataset'
]
...
class GraphConnectivityDataset(GraphDatasetBase):
    ...
    def __getitem__(self, item):
        ...
        return dict(
            n=graph.nr_nodes,
            relations=np.expand_dims(graph.get_edges(), axis=-1),
            target=graph.get_connectivity(
                self._dist_limit, exclude_self=False),
        )


class GraphTransitivityDataset(GraphDatasetBase):
    """The dataset for transitivity task in graphs."""
    ...
    def __getitem__(self, item):
        ...
        return dict(
            n=graph.nr_nodes,
            relations=np.expand_dims(graph.get_edges(), axis=-1),
            target=graph.get_transitivity(
                self._steps, self._paths, exclude_self=True),
        )


class GraphSelfDataset(GraphDatasetBase):
    """The dataset for self loop task in graphs."""
...
class FamilyTreeDataset(Dataset):
    ...
    def __getitem__(self, item):
        while len(self._data) == 0:
            ...
            if self._format == 'fmsd':
                relations = family.relations[:, :, 2:]
                ternaries = None
            elif self._format == 'tsd':
                relations = family.relations[:, :, 4:]
                ternaries = family.get_family_of_three()
                ternaries = np.expand_dims(ternaries, axis=-1)
            else:
            ...
            elif self._task == 'family-self':
                target = family.get_self()
            elif self._task == 'family-of-three':
                target = family.get_family_of_three()
            elif self._task == 'three-generations':
                target = family.get_three_generations()
            else:
                assert False, '{} is not supported.'.format(self._task)

            if not self._balance_sample:
                ret_dict = dict(n=family.nr_people, relations=relations, target=target)
